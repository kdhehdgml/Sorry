...
{
	m_scale = Vector3::Create(3.5f);
	m_baseRotation.y = D3DX_PI;
	
	SkinnedMeshHLSL* pSkinnedMesh = Objects::GetSkinnedMesh(path, filename);
...
void SkinnedMeshHLSL::Init()
{
	...
	//Load(ASSET_PATH + _T("Models/SiegeTank/"), _T("SiegeTank_Transform.X"));
	//Load(ASSET_PATH + _T("Models/boss/"), _T("Mutant.X"));
	//Load(ASSET_PATH + _T("Models/player/"), _T("combine_All.X"));
...
void SkinnedMeshHLSL::Render()
{
	...
	D3DXVECTOR4 camPos(g_pCamera->GetPosition(), 1);
...
void SkinnedMeshHLSL::DrawMeshContainer(LPD3DXFRAME pFrame)
{
	...
	D3DXMATRIXA16 *matView = g_pCamera->GetViewMatrix();
	...
	for (UINT iAttrib = 0; iAttrib < pMeshContainerEx->NumAttributeGroups; iAttrib++)
	{
		...
		for (UINT iPaletteEntry = 0; iPaletteEntry < pMeshContainerEx->NumPaletteEntries; ++iPaletteEntry)
		{
			...
			if (iMatrixIndex != UINT_MAX)
			{
				...
				g_pBoneMatrices[iPaletteEntry] = matTemp * m_worldMatrix;
			}
		}
		...
		D3DXCOLOR color1(pMeshContainerEx->vecMtlTex[pBoneComb[iAttrib].AttribId]->material.Ambient);
		D3DXCOLOR color2(.25, .25, .25, 1.0);
		D3DXCOLOR ambEmm;
		D3DXColorModulate(&ambEmm, &color1, &color2);
		ambEmm += D3DXCOLOR(pMeshContainerEx->vecMtlTex[pBoneComb[iAttrib].AttribId]->material.Emissive);

		// set material color properties 
		m_pEffect->SetVector("MaterialDiffuse",
			(D3DXVECTOR4*)&(
				pMeshContainerEx->vecMtlTex[pBoneComb[iAttrib].AttribId]->material.Diffuse));
		m_pEffect->SetVector("MaterialAmbient", (D3DXVECTOR4*)&ambEmm);

		// setup the material of the mesh subset - REMEMBER to use the original pre-skinning attribute id to get the correct material id
		m_pEffect->SetTexture("DiffuseMap_Tex", pMeshContainerEx->vecMtlTex[pBoneComb[iAttrib].AttribId]->pTexture);
