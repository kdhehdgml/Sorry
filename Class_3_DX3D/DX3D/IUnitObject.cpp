							...
							p->GetPosition())) < 4.0f)
						{
							...
							else
							{
								...
								while (Dir <3.5f && Dir > -3.5f)
								{
									...
									{
										targetPos = m_pos + ReDir * m_moveSpeed * 2.0f * m_currMoveSpeedRate;
										break;
									}
								}
								if (m_moveSpeed > 0 && abs(Dir) > 3.5f)
								{
									D3DXVECTOR3 thrust = D3DXVECTOR3(4.1f - abs(p->GetPosition().x - m_pos.x), 0,
										4.1f - abs(p->GetPosition().z - m_pos.z));
									if (p->GetPosition().x - m_pos.x >= 0)
									{
										if (p->GetPosition().z - m_pos.z >= 0)
											targetPos = m_pos - thrust;
										else
											targetPos = { m_pos.x - thrust.x, m_pos.y, m_pos.z + thrust.z};
									}
									else
									{
										if (p->GetPosition().z - m_pos.z >= 0)
...
void IUnitObject::GetScaleAndRotation(const D3DXMATRIX & val, D3DXVECTOR3 * OutScale, D3DXMATRIX& rotmat)
{
	...
	//rot 에 쿼터니언계산한값을 계산해준다!
	D3DXMatrixRotationQuaternion(&rotmat, &OutRotation);
