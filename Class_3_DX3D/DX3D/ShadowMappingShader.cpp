...
static D3DXHANDLE hWorldMatrix = "gWorldMatrix";
static D3DXHANDLE hWorldLightPos = "gWorldLightPos";
static D3DXHANDLE hLightViewMatrix = "gLightViewMatrix";
static D3DXHANDLE hLightProjMatrix = "gLightProjMatrix";
static D3DXHANDLE hViewMatrix = "gViewMatrix";
static D3DXHANDLE hProjMatrix = "gProjMatrix";
static D3DXHANDLE hWorldCamPos = "gWorldCameraPos";
...
{
	m_pCreateEffect = Shaders::Get()->GetShader(_T("resources/fx/CreateShadow.fx"));
	m_pApplyEffect = Shaders::Get()->GetShader(_T("resources/fx/ApplyShadow.fx"));
	
	const int SHADOWMAP_SIZE = 4096;
	D3DXCreateTexture(g_pDevice, SHADOWMAP_SIZE, SHADOWMAP_SIZE, 1,
		D3DUSAGE_RENDERTARGET, D3DFMT_R32F, D3DPOOL_DEFAULT, &m_pShadowRenderTarget);

	g_pDevice->CreateDepthStencilSurface(SHADOWMAP_SIZE, SHADOWMAP_SIZE, 
...
void ShadowMappingShader::Render()
{
	...
	g_pDevice->GetRenderTarget(0, &pHWRenderTarget);
	g_pDevice->GetDepthStencilSurface(&pHWDepthStencil);

	LPDIRECT3DSURFACE9 pShadowSurface = NULL;
	if (SUCCEEDED(m_pShadowRenderTarget->GetSurfaceLevel(0, &pShadowSurface)))
	{
		g_pDevice->SetRenderTarget(0, pShadowSurface);
		pShadowSurface->Release();
	}
	
	g_pDevice->SetDepthStencilSurface(m_pShadowDepthStencil);
	g_pDevice->Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0xFFFFFFFF, 1, 0);
	...
	g_pDevice->SetRenderTarget(0, pHWRenderTarget);
	g_pDevice->SetDepthStencilSurface(pHWDepthStencil);
